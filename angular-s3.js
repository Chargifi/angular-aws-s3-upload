	...
	angular.module('angular.aws.s3', []).
		...
		factory('S3UploaderSrv', ['$q', '$timeout', function($q, $timeout) {
			function process(file, $index, scope, ngModel) {
				...
				xhr.open('POST', '//' + scope.options.bucket + '.s3-' + scope.options.region + '.amazonaws.com/', true);
				xhr.send(fd);
				...
				function uploadComplete(e) {
					...
					if(xhr.status === 204) {
						file.real = window.location.protocol + '//' + scope.options.bucket + '.s3-' + scope.options.region + '.amazonaws.com/' + key;
						stop(true);
					} else {
						stop(false);
					}
